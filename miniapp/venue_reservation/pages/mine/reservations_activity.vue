					...
					<view class="reservation">
						...
						<view class="res-description">
							<view class="description-text">
								<text>预约人：{{item.visitorName}}\n身份证：{{item.idCardNumber.replace(/^(.{8})(?:\d+)(.{4})$/, "$1******$2")}}\n预约时间：{{item.activityTime}}</text>
							</view>
							<view class="description-button">
								<view v-show="showButtonScore" class="current-button">
									<u-button size="small" shape="circle" color="#ffaa00" @click="openPop(item.activityId,item.orderNumber)">
										评价
									</u-button>
								</view>
								
								<view v-show="showButtonCancel" class="current-button">
									<u-button size="small" shape="circle" color="#c8c9cc" @click="confirmCancel(item.activityId,item.orderNumber)">
										取消预约
									</u-button>
								</view>
								
								<view v-show="showButtonCancel" class="current-button">
									<u-button size="small" shape="circle" color="#19be6b" @click="visitActivity(item.orderNumber)">
										查看二维码
									</u-button>
								</view>
							</view>
		...
		<view class="pop-up">
			<u-popup :show="showPop" :round="10" @close="closePop">
				...
				<view class="rate-button" v-show="showConfirm">
					<u-button text="确定评分" shape="circle" color="#19be6b" @click="scoreAct"></u-button>
				</view>
			</u-popup>
		</view>
	</view>
	...
	export default {
		...
		methods: {
			...
			getList(index) {
				//获取预约列表函数
				uni.request({
					url: config.baseUrl+"sys/activityReservation/getActivityReservations",
					method: "GET",
					header: {
			...
			getStatus(ordernum){
				uni.request({
					url:config.baseUrl+"sys/activityReservation/getStatusAndScore",
					method:"GET",
					header: {
						token:uni.getStorageSync("token")
					},
			...
			scoreAct(){
				...
				uni.showModal({
						...
						if (res.confirm) {
							uni.request({
								url: config.baseUrl+"sys/activityReservation/scoreActivity",
								method:"GET",
								header: {
									token:uni.getStorageSync("token")
								},
			...
			confirmCancel(activityId,orderNumber){
				const self = this;
				uni.request({
					url:config.baseUrl+"sys/activityReservation/isDefaultCancel",
					method:"GET",
					header: {
						token:uni.getStorageSync("token")
					},
						...
						if(result.data.data){
							uni.showModal({
								...
								success: function(res) {
									if(res.confirm){
										uni.request({
											url:config.baseUrl+"sys/activityReservation/cancelDefault",
											method:"GET",
											header:{
												token:uni.getStorageSync("token")
											},
							...
							uni.showModal({
								...
								success: function(res) {
									if(res.confirm){
										uni.request({
											url:config.baseUrl+"sys/activityReservation/cancelNotDefault",
											method:"GET",
											header:{
												token:uni.getStorageSync("token")
											},
			...
			visitActivity(orderNumber){
				uni.request({
					url:config.baseUrl+"sys/activityReservation/visitActivity",
					method:"GET",
					header:{
						token:uni.getStorageSync("token")
					},
